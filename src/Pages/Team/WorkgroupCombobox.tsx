...
interface comboprops{
    data:workgroups[],
    value:number[],
    dataKey:keyof workgroups
    datavalue:keyof workgroups
    label:string,
    multiple?:boolean
    onchange?:(value: number[])=>void
}

const WorkgroupCombobox:React.FC<comboprops> = ({data,value,label,dataKey,datavalue,onchange}) => {
    const [isOpen,setisOpen] = useState<boolean>(false)
    const [activeList,setactiveList] = useState<workgroups[]>([])
    const [filteredList,setfilteredList] = useState<workgroups[]>([])
  ...
  return (
    <div>
        <div className="comboContainer relative w-full flex items-center flex-wrap rounded-[10px]" style={{border:"1px solid var(--color-gray-300)"}}>
            ...
            <div className="">
                <input type='search' onChange={handleSearch} className='text-[14px] px-2 py-2 outline-none border-none focus-visible:border-none shadow-none' placeholder={label} onFocus={()=>setisOpen(true)} onBlur={()=>setTimeout(() => {
                    setisOpen(false)
                }, 200)}/>
            </div>
           {
